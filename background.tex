...
In a profile, each \textit{Stereotype} needs to extend a UML concept (hereby referred to as \textit{base element} or \textit{meta-element}\SG{Why not just meta-element?}). 
...
These constraints can be rather long and need to be manually written and customised for \textit{each} edge \textit{Stereotype}. This can also be a labour-intensive and error-prone process.\SG{Will it help if we show an example OCL constraint for the given case?}
...
\sg{In the first option, for a Papyrus editor, the users can customise the palette using the palette customisation facility provided by Papyrus, the users are able to create a palette with custom creation tools in it and select what \textit{Stereotype}s should be instantiated for the tools.}\SG{Needs rewording.} 
...
\SG{It might be better to have light shaded elements in Fig.1 to support greyscale printing.}
\begin{itemize}
...
A few hundred lines of code need to be written while tedious Plug-in creation and repetitive model creation tasks should be done. \will{This is not necessarily the case, the claim is too strong} \thanos{This is supported by our evaluation on the archimate profile.}

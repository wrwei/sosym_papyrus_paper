...
business needs is steep.\thanos{Simos: maybe a reference is needed here.}
In contrast, Jorvik automates the process of generating UML profiles using a single annotated ECore\thanos{Will: make sure that we use a consistent word for Ecore. Somewhere we use Ecore somewhere ECore} metamodel and reduces significantly the developer's effort for specifying, designing and validating UML Papyrus profiles (cf. Section~\ref{sec:evaluation}).
...
facilitates the derivation of a UML profile using a simpler as input.\thanos{Simos: do you mean "a simpler input" or you forgot the word after simpler that denotes the file type?}

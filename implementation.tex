...
The source model of this transformation is the annotated Ecore \thanos{Sometimes we refer to this as ECore, sometimes as Ecore. Make sure we are consistent in the final version.} metamodel (e.g.,  Listing~\ref{lst:annotatedSdplEmfatic}) and the target model is a UML profile model.
...
Line 1 accesses the types \thanos{the "types" here is not clear. Is it UML:Class or Person/Tool. What about line 2?} that \emph{familiarWith} connects. 
...
For example, a \textit{Stereotype} that extends UML::Class needs to specialise the UML::Class \textit{MetamodelTypeConfiguration}; and it needs to specialise the Class Shape \textit{SpecializationTypeConfiguration} defined in the UMLDI \thanos{What is this? A reference is needed otherwise we need to explain. Or just discard this detail.} Element Types Configuration model. 
...
For elements of type \textit{Stereotype}, the connection of the SVG figure to the stereotypes is achieved by assigning the path of the SVG file to the \textit{svgFile} property available in CSS. \thanos{I don't understand this sentence (``For elements ... CSS").}
...
\subsection{Polishing Transformations (\#2b - \#5b)} \thanos{Any particular reason why the Architecture model doesn't support polishing transformations?}

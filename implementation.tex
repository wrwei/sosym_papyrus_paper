...
Figure~\ref{fig:transformationWorkflow} shows workflow of Jorvik. 
Each step in the workflow is identified by a number (\#2 -- \#9 in Figure~\ref{fig:transformationWorkflow}) for easier reference. 
In addition to the main steps, a pre-transformation validation script (\#1 in Figure~\ref{fig:transformationWorkflow}) is executed to verify the correctness of the annotations and provide useful feedback to the users if there is anything wrong. 
...
This transformation consists of two main rules, one that creates astereotype for each EClass element of the metamodel and a second that creates a \textit{Stereotype} for each EReference annotated as @Edge.
...
In Figure \ref{fig:sample_profile}, there are three stereotypes. ``Person" and ``Tool" extend meta-element UML::Class, and they correspond to classes ``Person" and ``Tool" in the metamodel shown in Listing \ref{lst:annotatedSdplEmfatic}. 
...
In this way, when an element is created in the diagram using the palette, behind the scene, Papyrus is able to locate the \textit{Stereotype} element and infer the UML syntax it specialises.
...
In EMF, a reference between to classes can be flagged as a containment relation, which is consistent with the \emph{containment} definition of UML associations (with exception of the deletion cascade mechanism).
These type of relations, when presented visually, can benefit from showing the contained elements shapes inside the shape of the container, as opposed to the line/arrow presentation.
...
Ideally, a custom profile editor should allow this containment relations to use the visual containment representation too.
...
Sadly, the current structure of the visual editor does not allow this functionality.
...
Further, these two sections are presented visually one after the other.
